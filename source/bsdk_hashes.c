...
inline uint64_t bsdk_fnv1(const uint8_t *data, size_t n)
{
...
inline uint64_t bsdk_fnv1a(const uint8_t *data, size_t n)
{
	uint64_t hash = FNV_OFFSET_BASE;
...
uint64_t bsdk_fnv1ext(const uint8_t *data, size_t n)
{
	uint64_t hash;
	uint64_t hash_a;
	...
	hash = bsdk_fnv1(data, n);
	BSDK_WRITE64BE(hash_bytes, hash);
	hash_a = bsdk_fnv1a(hash_bytes, 8);
	BSDK_WRITE64BE(hash_bytes, hash);
	...
	return bsdk_fnv1a(hash_bytes, 8);
}

void bsdk_merhash(uint8_t *digest, const uint8_t *data, size_t n)
{
	BSDK_WRITE64BE(digest, bsdk_fnv1ext(data, n));
	BSDK_WRITE64BE(digest+8, bsdk_fnv1ext(digest, 8));
}
