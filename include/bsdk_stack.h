...
struct bsdk_stack
{
    uint8_t* data;
    /**
     * The rw_off of the bsdk_stack as an offset that satisfies offset < stack_size
     */
    size_t rw_off;
    /**
 ...
 * @param self Pointer to a uninitialized bsdk_stack struct that is to be initialized
 * @param buff Backing buffer to use with this bsdk_stack
 * @param buff_len How long is the backing buffer
 */
extern void stack_init(struct bsdk_stack* self, uint8_t* buff, size_t buff_len);

/**
 * Pushes a value on to the rw_off of the `self` bsdk_stack.
 * @param self The bsdk_stack to push the value into
 ...
 */
extern int stack_push(struct bsdk_stack* self, uint8_t* buff, size_t buff_len);

/**
 * Removes a value off the rw_off of the bsdk_stack (it will remain in memory however).
 * @param self The bsdk_stack to pop the value from
 ...
 */
extern int stack_pop(struct bsdk_stack* self, uint8_t* buff, size_t buff_len);
